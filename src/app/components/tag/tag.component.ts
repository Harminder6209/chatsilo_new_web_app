...
export class TagComponent implements OnInit {
  ...
  selectedFbUserId: any;
  selectedProfilePic: string;
  selectedNumericFbId: any;
  ...
  constructor(
    ...
    this.colstags = [{ field: "name", header: "Tag Name" }];
    this.colsTagsForUser = [{ field: "name", header: "Tag Name" }];
    // this.colstag_user = [
    //   { field: 'profile_pic', header: 'image' },
    //   { field: 'fb_name', header: 'Name' },
    // ];
    this.colstag_user = [{ field: "fb_name", header: "Name" }];
  }
  ...
  ngOnInit(): void {
    ...
    this.csvExportName =
      "Tags (" + moment(new Date()).format("DD-MMMM-YYYY") + ")";
    // this.getTagCounter();
  ...
  tagList() {
    ...
    this.tagsService.getTags(token, fb_account).subscribe(
      (response: any) => {
  ...
  onRowReorder(event) {
    ...
    this.tagsService.redoderTag(token, data).subscribe(
      (response: any) => {
        if (response["status"] == 404) {
          this.spinner.hide();
          console.log(response);
  ...
  particularTag(id) {
    ...
    this.tagsService.getTagedUser(data, token).subscribe(
      (response: any) => {
        if (response["status"] == 404) {
          console.log(response);
        } else if (response["status"] == 200) {
          ...
          this.show_particularTagList_Nouser =
            tageduserTemp.length > 0 ? false : true;
          ...
          for (let i = 0; i < this.tageduser.length; i++) {
            var sameColorTags = this.tageduser[i].tags.find(
              (object) => object.id == tagId_filter
            );
  ...
  submit() {
    if (this.data.value.tag_name.trim()) {
      ...
      var token = localStorage.getItem("token");
      this.tagsService.addTag(token, data).subscribe(
        (response: any) => {
    ...
    sweetAlert
      ...
      .then((result) => {
        if (result.value) {
          this.spinner.show();
          var token = localStorage.getItem("token");
          this.tagsService.deleteTag(token, id).subscribe(
            (response: any) => {
              if (response["status"] == 404) {
                this.spinner.hide();
                console.log(response);
                ...
                // }
                sweetAlert
                  .fire("Deleted!", "This Tag is deleted.", "success")
                  .then((result) => {
                    if (result) {
                      // this.tags.splice(index, 1);
  ...
  submit_updateTag() {
    ...
    this.tagsService.updateTag(token, data, this.tagId).subscribe(
      (response: any) => {
        if (response.status == 200) {
  ...
  filterTagedUserList(acc_name) {
    ...
    if (account_name != "all") {
      ...
      this.show_particularTagList_Nouser =
        this.tageduser.length > 0 ? false : true;
    } else {
  ...
  openNote(fb_user_id, name) {
    ...
    var token = localStorage.getItem("token");
    this.contactService.getNotes(token, data).subscribe(
      (response: any) => {
        if (response["status"] == 404) {
    ...
    sweetAlert
      ...
      .then((result) => {
        if (result.value) {
          this.spinner.show();
          var token = localStorage.getItem("token");
          this.contactService.deleteNotes(token, id).subscribe(
            (response: any) => {
              if (response["status"] == 404) {
                this.spinner.hide();
                console.log(response);
                ...
                sweetAlert.fire("Deleted!", "This Note is deleted.", "success");
                console.log(response);
              }
            },
            (err) => {
  ...
  updateNote(id, fb_user_id, noteText) {
    ...
    var token = localStorage.getItem("token");
    this.contactService.updateNotes(token, data, id).subscribe(
      (response: any) => {
        if (response.status == 200) {
  ...
  eventHandler(event: string[]) {
    ...
    let latestTags = [];
    event.forEach((item:any,index) => { 
      let tagId = item.id;

      let tagIndex = previousTags.findIndex((x) => x.id === tagId);
      item.users = 0;
      item.order_num = 0;
      if(tagIndex >= 0){
        item.users = previousTags[tagIndex].users;
        item.order_num = previousTags[tagIndex].order_num;
      }
      latestTags.push(item);
    });
    this.tags = latestTags;
  // this.tags = event;
    this.emptyTags = this.tags.length > 0 ? false : true;
  ...
  showTags(tags, id, fb_name, fb_user_id, profile_image, numeric_fb_id) {
    this.selectedFbName = fb_name;
    this.selectedFbUserId = fb_user_id;
    this.selectedProfilePic = profile_image;
    this.selectedNumericFbId = numeric_fb_id;
  ...
  onSelect(taggedId, event) {
    console.log(event);
    console.log(this.selectedTags);
    var tageduser_index = this.tageduser.findIndex(
    ...
    //   tagIds: tagIds,
    //   fbName: this.selectedFbName,
    //   fbUserId: this.selectedFbUserId,
    //   profilePic: this.selectedProfilePic,
    //   accountFbId: fb_account,
    //   numericFbId: this.selectedNumericFbId,
    // };
    // var token = localStorage.getItem("token");
    ...
    // this.contactService.updateTags(token, data).subscribe(
    //   (response: any) => {
    //     if (response.status == 200) {
    //     } else if (response.status == 404) {
    //     }
      ...
      (response: any) => {
        if (response.status == 200) {
          ...
          this.msgs.push({
            severity: "success",
            summary: "Success Message",
            detail: "Tags assigned successfully",
          });
          setTimeout(() => {
            this.msgs = [];
          }, 3000);
  ...
  getTagCounter() {
    ...
    this.tagsService.tagsCounter(token, data).subscribe(
      (response: any) => {
        if (response.status == 200) {
  ...
  colorChanged(event, tag_id) {
    ...
    this.tagsService.changeTagColor(token, data).subscribe(
      (response: any) => {
        // console.log(response)
      },
      (err) => {
