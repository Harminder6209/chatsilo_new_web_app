...
export class ProfileComponent implements OnInit {
  ...
  getprofile() {
    ...
    this.refreshTokenService.getAccountList(token).subscribe(
      (response: any) => {
    ...
    sweetAlert
      ...
      .then((result) => {
        if (result.value) {
          ...
          this.profileService.deleteAccount(token, data).subscribe(
            (response: any) => {
              if (response["status"] == 404) {
                console.log(response);
              } else if (response["status"] == 200) {
                ...
                // console.log(response);
                this.deleteLinkedAccount = fb_account_id;
                
                /* remove account from sidebar dropdown */
                let linkedAccounts = this.SharedService.getFbAccountsObservableValue();
                let indexToRemove = linkedAccounts.findIndex((x) => x.value == fb_account_id);
                linkedAccounts.splice(indexToRemove, 1);
                this.SharedService.updateFbAccountsObservable(linkedAccounts);
    ...
    sweetAlert
      ...
      .then((result) => {
        if (result.value) {
          ...
          this.profileService.makeAccountPrimary(token, data).subscribe(
            (response: any) => {
              if (response["status"] == 404) {
                console.log(response);
              } else if (response["status"] == 200) {
