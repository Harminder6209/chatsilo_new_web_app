...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.SharedService.fbAccountsObservable.subscribe((data) => {
  ...
  verifyUserToken() {
    ...
    this.refreshTokenService.refreshToken(tokenTemp).subscribe(
      (response: any) => {
      ...
      (response: any) => {
        if (response["status"] == 404) {
          ...
          for (let i = 0; i < accountListTemp.length; i++) {
            dropdownItems.push({
              label: accountListTemp[i].fb_account_id,
              value: accountListTemp[i].fb_account_id,
            });
            if(i == accountListTemp.length - 1){
              this.SharedService.updateFbAccountsObservable(dropdownItems);
            }
            
          }

          // setTimeout(() => {
          //   console.log('triggered to update observable')
  ...
  updateLinkedAccount(fb_account_id) {
    this.ngOnInit();
    console.log("updateLinkedAccount");
